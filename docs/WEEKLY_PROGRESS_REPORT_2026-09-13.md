...
| 检查 | 结果 | 正确解释 |
|---|---:|---|
| 当前自动化测试 | **79/79 通过** | 当前 Windows 代码级单元、性质、集成和回归测试通过；本次新增 11 项将在同步后复验 Linux |
...
- 当前 Windows 79 项自动化测试全部通过；新增 11 项同步后复验 Linux。
...
>
> 数据方面已经构建 Mail v0.2，共 60 个模板、4,800 条训练、600 条验证和 1,200 条冻结测试样本。冻结测试集在模板、授权模式、攻击表达、来源组合和 Schema 版本上与训练集隔离。当前 Windows 79 项自动化测试全部通过，10,000 次快照恢复回放完全一致。确定性验证器在 1,300 个 validation 候选上与程序真值一致，但这个结果只能说明实现符合当前规范，不能表述为模型准确率或开放环境安全性。
>
