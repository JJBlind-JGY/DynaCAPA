...
| 检查 | 结果 | 正确解释 |
|---|---:|---|
| 当前自动化测试 | **82/82 通过（Windows）** | Linux 已通过前 79 项；最新六模式先导的 3 项测试待本次同步复验 |
...
| Balanced SFT64 | 100% | 100% | 0.4661 | 100% | 0% | 0% | 三模式等量训练后转为 `ask` 偏置：50 ask、1 block、9 execute |
| Balanced SFT64 + contrastive DPO32 | 91.7% | 15.0% | 0.0460 | 0% | 0% | 85.0% | 偏好准确率 100%，但结构语言严重崩坏；51 条无效 |
| 上述模型 + chosen-SFT loss | 90.0% | 71.7% | 0.4245 | 0% | 0% | 28.3% | 部分恢复结构和 ask/block 区分，但无合法 execute；停止该 DPO 分支 |

\* Base/SFT8/DPO8 的 UPR=0 不能解释为安全，因为它们没有 Schema-valid
动作；对应的 format-or-safety failure rate 均为 100%。该表是分层平衡的
同一组 60 条工程先导样本，不是总体加权结果，也不是 Gate B 结果。最后
一行是预注册的单变量机制诊断，不进行事后 loss 权重搜索。
...
- Windows 的 82 项自动化测试全部通过；Linux 已通过前 79 项，最新 3 项同步后复验。
...
>
> 数据方面已经构建 Mail v0.2，共 60 个模板、4,800 条训练、600 条验证和 1,200 条冻结测试样本。冻结测试集在模板、授权模式、攻击表达、来源组合和 Schema 版本上与训练集隔离。当前 Windows 的 82 项自动化测试全部通过，10,000 次快照恢复回放完全一致。确定性验证器在 1,300 个 validation 候选上与程序真值一致，但这个结果只能说明实现符合当前规范，不能表述为模型准确率或开放环境安全性。
>
...
| `data/review/mail_v0_3_six_mode_semantic_pilot/review_items.jsonl` | 明天可抽查的 60 条六模式盲审项；当前不可训练 |
| `data/review/mail_v0_3_six_mode_semantic_pilot/answer_key.jsonl` | 与盲审项分离的目标、证据和期望动作 |
