...
| 1. SFT/DPO | 模型能否稳定生成合法、完整且不坍缩的策略输出？ | Qwen3-0.6B 冒烟；Qwen3-4B LoRA SFT/DPO | Gate B | 尚未开始 |
...
冻结测试集与训练集已在模板、授权模式、攻击表达、来源组合和 Schema 版本五个维度上隔离。训练数据覆盖 `execute / ask / sandbox / rewrite / block / stop` 六种合法模式，避免上一版本存在的模式覆盖缺口。

### 3.4 自动化验证结果

| 检查 | 结果 | 正确解释 |
|---|---:|---|
| 当前自动化测试 | **41/41 通过** | 代码级单元、性质、集成和回归测试通过 |
...
- AuthGraph-style 与 ARGUS-style 邻近基线尚未实现。

因此，训练开关仍保持关闭，**目前尚未开展 SFT、DPO、DACPO 或 VICC 训练**。这不是进度遗漏，而是按照研究门控避免在不可靠标签和环境上消耗服务器算力。

## 5. 接下来各阶段要做什么

### 最近 1–2 周：完成 Gate A

1. 完成 600 条主审与 60 条独立复审，冻结标注后再进行裁决。
2. 计算人工语义指标、置信区间和评审一致性，定位真值争议类别。
3. 实现 AuthGraph-style 与 ARGUS-style 的公平邻近基线。
4. 扩充撤销、确认、范围与 Schema 漂移的对抗回归案例。
5. Gate A 达标后，才解锁模型训练配置。

### 随后 3 周左右：SFT/DPO 与 Gate B

1. 在本地使用 Qwen3-0.6B 做格式和数据管线冒烟。
2. 在 Linux 服务器对 Qwen3-4B 进行 LoRA SFT，再做标准 DPO。
3. 评估 JSON 合法率、字段完整率、IID/OOD PVR、mode macro-F1 和模式坍缩。
4. Gate B 通过后进入在线 RL；失败则优先修复数据、格式或策略覆盖。
...
> 目前我没有直接开始大模型训练，而是先完成了动态授权安全实验所需的最小可信闭环。现阶段已经实现 Mail 模拟环境、动态授权引擎、工具契约、Proof Checker、确定性 Verifier、Shield、状态快照与可回放轨迹，并且把模型候选动作和 Shield 实际执行动作分离，保证后续强化学习不会发生信用混淆。
>
> 数据方面已经构建 Mail v0.2，共 60 个模板、4,800 条训练、600 条验证和 1,200 条冻结测试样本。冻结测试集在模板、授权模式、攻击表达、来源组合和 Schema 版本上与训练集隔离。当前 41 项自动化测试全部通过，10,000 次快照恢复回放完全一致。确定性验证器在 1,300 个 validation 候选上与程序真值一致，但这个结果只能说明实现符合当前规范，不能表述为模型准确率或开放环境安全性。
>
> 现在处于 Gate A 阶段。下一步先完成 600 条主审和 60 条独立复审，得到人工语义指标，并补充 AuthGraph 和 ARGUS 风格基线。Gate A 通过后，才会在服务器上依次开展 Qwen3-4B 的 SFT、DPO 和 DACPO。后续是否把 VICC 做到在线训练，以及会议论文最终强调安全系统、策略内化还是反事实信用，将由 Gate C 和 Gate D 的实验结果决定。
