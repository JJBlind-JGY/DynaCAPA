...
Allowed `mode_label` values are `execute`, `ask`, `sandbox`, `rewrite`, `block`, and `stop`. Boolean fields use `true` or `false`. Confidence uses `high`, `medium`, or `low`. Reason codes must come from `dynacapa.core.enums.ReasonCode`.
